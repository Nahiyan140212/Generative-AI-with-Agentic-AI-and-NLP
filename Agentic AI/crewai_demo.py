...
class EuriLLMWrapper(LLM):
    ...
    def bind(self, **kwargs):
        """Create a new LLM with the specified kwargs merged with existing parameters."""
        ...
        for key, value in kwargs.items():
            setattr(new_instance, key, value)
        # Return a runnable that uses the new instance for calls
        return RunnableLambda(lambda x: new_instance._call(x["input"] if isinstance(x, dict) else x))
